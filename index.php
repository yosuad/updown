	...
	if($core->FileTypeVerification($_FILES["fileToUpload"])){
		if($core->FileSizeVerification($_FILES["fileToUpload"])){
			...
			if($core->UploadFile($_FILES["fileToUpload"], $newfilename)){
				...
				// ✅ Solo codificamos el nombre del archivo
				$destination = base64_encode($newfilename);

				// ✅ Usamos el dominio correcto para el link
				$downloadLink = "https://files.wiquiweb.com/download.php?file=" . $destination;
				?>
				...
				<div class="notification error">
					❌ An error occurred while trying to upload your file(s).
				</div>
				<?php
			}
		}else{
			?>
			<div class="notification error">
				⚠️ Your file is too large or too small.
			</div>
			<?php
		}
	}else{
